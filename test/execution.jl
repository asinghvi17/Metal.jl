...
@testset "launch configuration" begin
    ...
    @metal grid=1 dummy()
    @metal grid=(1,1) dummy()
    @metal grid=(1,1,1) dummy()

    @test_throws InexactError @metal threads=(-2) dummy()
    @test_throws InexactError @metal grid=(-2) dummy()
...
@testset "reflection" begin
    ...
    @test occursin("julia_dummy", sprint(io->(@device_code_llvm io=io optimize=false @metal dummy())))
    @test occursin("julia_dummy", sprint(io->(@device_code_llvm io=io @metal dummy())))
    @test occursin("julia_dummy", sprint(io->(@device_code_air io=io @metal dummy())))
    @test_broken occursin("julia_dummy", sprint(io->(@device_code_agx io=io @metal dummy())))
